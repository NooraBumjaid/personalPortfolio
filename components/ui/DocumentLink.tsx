...
export function getDocumentGridClassName(count: number) {
  if (count === 2) {
    return "flex flex-nowrap items-center justify-center gap-1.5";
  }
...
export function DocumentLink({ label, url }: DocumentLinkProps) {
  return (
    <a
      ...
      className="group relative inline-flex max-w-full shrink-0 items-center gap-1.5 overflow-hidden rounded-full border border-cyber-accent/30 bg-gradient-to-r from-cyber-accent/[0.08] via-white/[0.04] to-transparent py-0.5 pl-0.5 pr-2.5 shadow-[0_2px_10px_rgba(0,0,0,0.16),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-cyber-accent/60 hover:from-cyber-accent/[0.14] hover:shadow-[0_6px_18px_rgba(34,211,238,0.16),inset_0_1px_0_rgba(255,255,255,0.12)] active:scale-[0.98] sm:gap-2 sm:pr-3"
    >
      ...
      <span className="whitespace-nowrap pr-0.5 text-[10px] font-semibold tracking-wide text-cyber-text transition-colors duration-300 group-hover:text-cyber-accent sm:text-xs">
        {label}
      </span>
    </a>
  );
