...
export function ActivitiesTimeline() {
  return (
    <div className="relative">
      <div className="absolute left-4 top-0 hidden h-full w-px bg-gradient-to-b from-cyber-accent/40 via-cyber-border to-transparent md:left-1/2 md:block" />
      ...
      <MotionStagger className="space-y-6">
        {leadership.map((item, index) => (
          <MotionItem key={item.role + item.period}>
            ...
            >
              <div className="absolute left-4 top-6 hidden h-3 w-3 -translate-x-1/2 rounded-full border-2 border-cyber-accent bg-cyber-bg md:left-1/2 md:block" />
              <GlassCard hover className="w-full p-5 md:w-[calc(50%-2rem)] md:ml-0">
