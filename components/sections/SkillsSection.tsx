...
const categoryStyles: Record<SkillCategoryKey, { gradient: string; glow: string }> = {
  cybersecurity: {
...
export function SkillsSection() {
  const { skillCategories, ui } = useLocale();
  const categories = Object.entries(skillCategories) as [SkillCategoryKey, (typeof skillCategories)[SkillCategoryKey]][];
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <MotionSection>
          <SectionHeader
            label={ui.skillsLabel}
            title={ui.skillsTitle}
            highlight={ui.skillsHighlight}
              ...
              <MotionItem key={key}>
                <article
                  className={cn(
                    ...
                    styles?.glow && `hover:shadow-xl ${styles.glow}`
                  )}
                >
                  <div
                    className={cn(
                      ...
                      styles?.gradient ?? "from-white/5 to-transparent"
                    )}
                  />
